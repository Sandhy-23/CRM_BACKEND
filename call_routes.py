...
def get_calls(current_user):
    calls = Call.query.filter_by(organization_id=current_user.organization_id).order_by(Call.timestamp.desc()).all()
    return jsonify([{
        "id": c.id,
        "name": c.name,
        "phone": c.phone,
...
def create_call(current_user):
    ...
    db.session.commit()
    return jsonify({"message": "Call logged", "id": new_call.id}), 201